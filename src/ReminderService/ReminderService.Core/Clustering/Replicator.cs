...
{
	public class Replicator : IConsume<ClusterMessage.Replicate<ReminderMessage.Schedule>>
	{
		...
		public void Handle(ClusterMessage.Replicate<ReminderMessage.Schedule> msg)
		{
					...
					response => {
						if (response.ResponseStatus == ResponseStatus.Completed)
						{
							if (response.StatusCode != HttpStatusCode.Created)
								OnHttpError(msg.Reminder, response);
						}
						else
							OnTransportError(msg.Reminder, response);
					},
					exception => OnException(msg.Reminder, exception),
					() => OnReplicated(msg.Reminder)
				);
		}
		...
		{
			_bus.Send (new ClusterMessage.Replicated<ReminderMessage.Schedule> (msg));
		}
