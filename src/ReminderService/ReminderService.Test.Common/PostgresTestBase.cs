...
namespace ReminderService.Test.Common
{
	public abstract class PostgresTestBase
	{
		...
		protected void AssertUndeliverable(Guid reminderId)
		{
			var commandText = 
				string.Format("SELECT count(*) FROM public.reminders WHERE reminder_id = '{0}' AND cancelled = FALSE AND sent_time IS NULL AND undeliverable = TRUE",
					reminderId);
			using (var connection = new NpgsqlConnection (ConnectionString)) {
				using (var command = new NpgsqlCommand (commandText, connection)) {
					connection.Open ();
